...
func (d *NodeServer) NodeGetVolumeStats(ctx context.Context, req *csi.NodeGetVolumeStatsRequest) (*csi.NodeGetVolumeStatsResponse, error) {
	...
	if err == nil {
		if !exists {
			glog.Info("Volume path not exists", "volumePath", volumePath)
			return nil, status.Error(codes.NotFound, "Volume path not exists")
		}
		if err := local.WaitMount(volumePath, defaultCheckTimeout); err != nil {
			glog.Info("Check volume path is mountpoint failed", "volumePath", volumePath, "error", err)
			return nil, status.Errorf(codes.Internal, "Check volume path is mountpoint failed: %s", err)
		}
	} else {
		glog.Info("Check volume path %s, err: %s", "volumePath", volumePath, "error", err)
